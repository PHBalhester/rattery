...
(async()=>{
 ...
 for(const software of [false]){
  ...
  await p.evaluate(async url=>{window.__sim=await import(url);window.__base=structuredClone(window.__sim.getWorld().rats);const {marketBus}=await import('/src/market/bus.ts');let n=0;setInterval(()=>marketBus.emit({id:'QA-perf-'+n,ts:Date.now(),side:n++%2?'buy':'sell',usd:4000,eth:4000/2400,tokens:1,trader:'QA-load',isNewHolder:false,venue:'demo'}),20)},storeUrl);
  for(const population of [80]){
   ...
   const {profile}=await cdp.send("Profiler.stop");fs.writeFileSync(__dirname+"/../test-results/audit-2026-09-16/cpu-profile.json",JSON.stringify(profile));
 ...
 fs.writeFileSync(__dirname+'/render-active-benchmark.json',JSON.stringify({scope:'1280x720, active simulation and requested 50 synthetic trades/s, overview camera; default renderer.',results},null,2));console.log(JSON.stringify(results));
