import {CARE_ERRORS} from '../careErrors';
...
const explanations:Record<CareAction,[string,string]>={
 ...
 play:['Encourage play, use 3 energy points and reduce cortisol by up to 3 points. Wait until the rat finishes social grooming or caring for pups before starting. Cooldown: 2 real hours.','鼓励玩耍，消耗3点能量并降低最多3点皮质醇。大鼠必须处于可玩耍状态。'],
...
class CareRequestError extends Error {}
async function api(op:string,data:unknown={}){
 ...
 if(!response.ok){
  ...
  if(typeof result.code==='string'&&Object.prototype.hasOwnProperty.call(CARE_ERRORS,result.code))throw new CareRequestError(CARE_ERRORS[result.code]);
  if(response.status===401)throw new CareRequestError('Your session expired. Reconnect your wallet and sign in again.');
...
export default function PaidCare({ratId}:{ratId:string|null}){
 ...
 useEffect(()=>{if(notice)feedback.current?.scrollIntoView({block:'nearest',behavior:'smooth'});},[notice]);
 ...
 const rat=view?.rats.find(r=>r.id===ratId),current:CareAction=rat?.owner&&action==='mint'?'name':!rat?.owner&&action==='name'?'mint':action,simRat=ratId?getWorld().rats[ratId]:undefined,suggested=simRat?ratNeeds(simRat).flatMap(n=>n.actions):[],pending=view?.intents.filter(i=>['reserved','review'].includes(i.status))??[],intent=pending.find(i=>i.id===selected)??pending.find(i=>i.rat_id===ratId);
 ...
 async function run(work:()=>Promise<void>){if(locked.current)return;locked.current=true;setBusy(true);setNotice('');try{await work();}catch(error){if(error instanceof CareRequestError){setNotice(error.message);}else if(error instanceof Error&&['Offensive name','Invalid name'].includes(error.message)){setNotice(tr('Choose another name. Racist, hateful or offensive names are not allowed.','请选择其他名字。禁止种族歧视、仇恨或冒犯性名字。'));}else setNotice(tr('Could not complete. Refresh your account; if a transaction was sent, recover it without paying again.','未能完成。请刷新账户；若交易已发送，请恢复原交易，不要重复支付。'));}finally{try{await refresh();}catch{}locked.current=false;setBusy(false);}}
 const reserve=()=>run(async()=>{if(!rat)throw Error('Select rat');const chosen=current;if(['mint','name'].includes(chosen))assertAllowedName(name.trim()||rat.name);const key=JSON.stringify([rat.id,chosen,name]);if(request.current?.key!==key)request.current={key,id:crypto.randomUUID()};const i=await api('reserve',{requestId:request.current.id,ratId:rat.id,action:chosen,...(['mint','name'].includes(chosen)?{name:name.trim()||rat.name}:{})}) as Intent;validate(i);setSelected(i.id);setNotice(tr('Reserved. Review the amount, then confirm.','已预留，请核对金额后确认。'));});
 const pay=()=>run(async()=>{if(!intent||intent.status!=='reserved')throw Error('No reservation');validate(intent);if(['mint','name'].includes(intent.action))assertAllowedName(intent.name);if(intent.cost===0){await api('finalize',{id:intent.id});setNotice(tr('Name updated.','名字已更新。'));return;}
...
{!intent.submission_started_at&&intent.status==='reserved'&&<button className="chip" disabled={busy} onClick={()=>void run(async()=>{await api('cancel',{id:intent.id});request.current=null;setSelected('');})}>{tr('Cancel','取消')}</button>}
 ...
 </div>:rat&&!rat.dead?<><p>{rat.owner?rat.owner===w.account?tr('You own this rat.','您拥有这只大鼠。'):tr('Only the owner can provide care.','仅所有者可提供照护。'):tr('Unminted · any wallet may provide care.','尚未铸造 · 任何钱包均可照护。')}</p><CareActionPicker owned={!!rat.owner} selected={current} suggested={suggested} disabled={a=>busy||(a!=='snake'&&!!(rat.owner&&rat.owner!==w.account))} onPick={a=>{setAction(a);setNotice('');}}/>{['mint','name'].includes(current)&&<label>{tr('Name','名字')}<input maxLength={24} value={name} onChange={e=>setName(e.target.value)} placeholder={rat.name} aria-invalid={!!name&&!!nameIssue(name)}/><small>{tr('No racist, hateful or offensive names.','禁止种族歧视、仇恨或冒犯性名字。')}</small>{name&&nameIssue(name)&&<span role="alert">{tr('Please choose another name.','请选择其他名字。')}</span>}</label>}<p>{tr('Paid actions burn RATTERY. Ownership is recorded in the project, without an NFT.','付费操作会销毁RATTERY，所有权记录在项目内，不发行NFT。')}</p><p className="care-explanation care-selected-explanation"><b>{CARE_ICONS[current]} {tr(...labels[current])} · {CARE_RULES[current].cost?CARE_RULES[current].cost.toLocaleString(locale())+' RATTERY':tr('Free','免费')}</b>{tr(...explanations[current])}</p><button className="chip chip-accent care-review" disabled={busy||(action!=='snake'&&!!(rat.owner&&rat.owner!==w.account))} onClick={()=>void reserve()}>{tr('Review action & cost','核对操作与费用')}</button></>:<p>{tr('Select a living rat to view actions. Pending payments remain recoverable here.','选择存活的大鼠以查看操作，待处理支付仍可在此恢复。')}</p>}
 ...
 <p ref={feedback} role="status">{busy?tr('Working…','处理中…'):notice}</p></section>;
}
