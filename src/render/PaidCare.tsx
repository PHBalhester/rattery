...
{!intent.submission_started_at&&intent.status==='reserved'&&<button className="chip" disabled={busy} onClick={()=>void run(async()=>{await api('cancel',{id:intent.id});request.current=null;setSelected('');})}>{tr('Cancel','取消')}</button>}
 ...
 </div>:rat&&!rat.dead?<><p>{rat.owner?rat.owner===w.account?tr('You own this rat.','您拥有这只大鼠。'):tr('Only the owner can provide care.','仅所有者可提供照护。'):tr('Unminted · any wallet may provide care.','尚未铸造 · 任何钱包均可照护。')}</p><label>{tr('Action','操作')}<select value={rat.owner&&action==='mint'?'name':action} onChange={e=>setAction(e.target.value as CareAction)} disabled={busy}>{[['mint','name','feed','water','pet','play','treat','explore'],['prosocial','aggression'],['snake']].map((group,i)=><optgroup key={i} label={i===0?tr('Naming & care','命名与照护'):i===1?tr('Advanced effects','高级效果'):tr('Danger · can kill a rat','危险 · 可能杀死大鼠')}>{(group as CareAction[]).filter(a=>rat.owner?a!=='mint':a!=='name').map(a=><option key={a} value={a}>{tr(...labels[a])} · {CARE_RULES[a].cost.toLocaleString(locale())} RATTERY</option>)}</optgroup>)}</select></label>{['mint','name'].includes(action)&&<label>{tr('Name','名字')}<input maxLength={24} value={name} onChange={e=>setName(e.target.value)} placeholder={rat.name} aria-invalid={!!name&&!!nameIssue(name)}/><small>{tr('No racist, hateful or offensive names.','禁止种族歧视、仇恨或冒犯性名字。')}</small>{name&&nameIssue(name)&&<span role="alert">{tr('Please choose another name.','请选择其他名字。')}</span>}</label>}<p>{tr('Paid actions burn RATTERY. Ownership is recorded in the project, without an NFT.','付费操作会销毁RATTERY，所有权记录在项目内，不发行NFT。')}</p>{action==='snake'&&<p className="care-explanation">{tr(...explanations.snake)}</p>}{action==='play'&&<p className="care-explanation">{tr(...explanations.play)}</p>}{action==='pet'&&<p className="care-explanation">{tr('Gentle petting reduces acute stress by up to 5 points, depending on receptivity. Cooldown: 1 hour. A heart appears above the rat after confirmation.','温柔抚摸可根据接受程度降低最多5点急性压力。冷却时间为1小时，确认后大鼠上方会显示爱心。')}</p>}{['prosocial','aggression'].includes(action)&&<p>{tr('Fictional behavioural effect for adults: 2 real hours, shared cooldown, no stacking.','仅适用于成年个体的虚构行为效果：持续现实2小时，共享冷却时间，不可叠加。')}</p>}<button className="chip" disabled={busy||(action!=='snake'&&!!(rat.owner&&rat.owner!==w.account))} onClick={()=>void reserve()}>{tr('Review action & cost','核对操作与费用')}</button></>:<p>{tr('Select a living rat to view actions. Pending payments remain recoverable here.','选择存活的大鼠以查看操作，待处理支付仍可在此恢复。')}</p>}
