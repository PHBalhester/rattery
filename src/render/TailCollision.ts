...
for(const wall of refugeWalls){const c=world(wall.x,wall.y);barriers.push({ax:c.x,az:c.z-wall.halfY/30,bx:c.x,bz:c.z+wall.halfY/30,radius:wall.halfX/30+.03,bottom:-.12,top:1.15});}
...
export function projectTailPoint(p:T.Vector3,radius:number,ground:Ground,scenery=true){
 const q=p.clone();q.y=Math.max(q.y,ground(q.x,q.z)+radius);
 if(scenery)for(const b of grid.get(Math.floor(q.x)+':'+Math.floor(q.z))??[]){
...
export class TailCollision {
 ...
 update(dt:number,ground:Ground,enabled:boolean,reset:boolean,scenery=true){
