...
export class BlenderRatVisual{
 ...
 update(dt:number,distance:number,moving:boolean,speed:number,reduced:boolean,reset:boolean,eyesOpen:boolean,pregnant:boolean,social:boolean,ground?:Ground,treadmill=false,wheel=false,environmentCollision=false){
  this.model.position.x=wheel?-.10:0;
  this.model.position.y=wheel?.09:-.02;
  const level=distance<12?0:distance<26?1:2;
  ...
  this.bodyMotion.update(dt,moving||treadmill?speed:0,this.motor.diagnostics(),false,reduced,reset,wheel);
  this.naturalMotion.update(dt,moving||treadmill||social,reduced,reset);
  this.model.scale.z=.8*(pregnant?1.13:1);
  const flat=this.root.parent?.position.y??0;
  this.motor.update(dt,ground??(()=>flat),reduced,reset,treadmill?speed:0);

  this.tailCollision.update(dt,ground??(()=>flat),environmentCollision&&!wheel,reset);
