import {socialDens,inCourtyard} from './courtyard.js';
...
export const zones=[{name:'nest',x:NEST_POS.x,y:NEST_POS.y,capacity:18},...habitatRoutes.map((r,i)=>({name:`chamber ${i+1}`,x:r[81].x,y:r[81].z,capacity:14})),{name:'courtyard',x:800,y:730,capacity:28},...socialDens.map((d,i)=>({...d,name:`social den ${i+1}`}))];
export function zoneOf(r:{x:number;y:number}){
 for(let i=0;i<socialDens.length;i++){const d=socialDens[i];if(Math.abs(r.x-d.x)<66&&Math.abs(r.y-d.y)<65)return 7+i;}
 if(inCourtyard(r)&&r.y>610)return 6;
 let best=0;for(let i=1;i<6;i++)if(Math.hypot(r.x-zones[i].x,r.y-zones[i].y)<Math.hypot(r.x-zones[best].x,r.y-zones[best].y))best=i;return best;
}
...
export function ecologyStep(w:World,dt:number){
 ...
 const atResource=z<6&&(w.habitatMode!=='basic'||z===0)&&Math.hypot(r.x-zones[z].x,r.y-zones[z].y)<65,access=atResource?Math.min(1,6/Math.max(1,near.length+1)):0;
 ...
 // A brief solo excursion is not sustained social deprivation. Recovery is gradual.
 const company=living.some(q=>q!==r&&Math.hypot(q.x-r.x,q.y-r.y)<150&&affinity(w,r,q)>-.35);
 state.isolationDays=Math.max(0,Math.min(8,(state.isolationDays??0)+dt*(company?-4:1)));
 state.isolationDistress=clamp((state.isolationDays-1)/5);
 const isolation=state.isolationDistress*.3;
